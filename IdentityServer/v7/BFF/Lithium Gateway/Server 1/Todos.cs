...
public static class TodoEndpointGroup
{
    ...
    private static readonly List<ToDo> data = new List<ToDo>()
        {
            new ToDo { Id = ToDo.NewId(), Date = DateTimeOffset.UtcNow, Name = "SERVER 1 - Demo ToDo API", User = "2 (Bob Smith)" },
            new ToDo { Id = ToDo.NewId(), Date = DateTimeOffset.UtcNow.AddHours(1), Name = "SERVER 1 - Stop Demo", User = "2 (Bob Smith)" },
            new ToDo { Id = ToDo.NewId(), Date = DateTimeOffset.UtcNow.AddHours(4), Name = "SERVER 1 - Have Dinner", User = "1 (Alice Smith)" },
        };
    ...
    public static RouteGroupBuilder ToDoGroup(this RouteGroupBuilder group)
    {
        ...
        group.MapPost("/", (ToDo model, ClaimsPrincipal user, HttpContext context) =>
        {
            ...
            model.Name = "SERVER 1 - " + model.Name; // "SERVER 1 -
            data.Add(model);
